...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row = [inv.name,inv.company_branch, inv.posting_date, inv.supplier, inv.supplier_name]
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = [
		...
		_("Company Branch") + ":Link/Company Branch:130",
...
def get_conditions(filters):
	...
	if filters.get("company_branch"): conditions += " and company_branch=%(company_branch)s"
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select
			name,company_branch, posting_date, credit_to, supplier, supplier_name, tax_id, bill_no, bill_date,
