...
from branch.branch.utils import get_currency_precision
...
class ReceivablePayableReport(object):
		...
		for gle in self.gl_entries:
			...
			if not key in self.voucher_balance:
				self.voucher_balance[key] = frappe._dict(
					...
					company_branch = gle.company_branch,
					party = gle.party,
	...
	def get_gl_entries(self):
		...
		self.gl_entries = frappe.db.sql("""
			select
				name, posting_date,company_branch, account, party_type, party, voucher_type, voucher_no, cost_center,
	...
	def add_common_filters(self, conditions, values, party_type_field):
		...
		if self.filters.company_branch:
			conditions.append("company_branch=%s")
			values.append(self.filters.company_branch)
	...
	def get_columns(self):
		...
		self.add_column(label=_('Company Branch'), fieldname='company_branch', fieldtype='Link', options='Company Branch', width=150)
