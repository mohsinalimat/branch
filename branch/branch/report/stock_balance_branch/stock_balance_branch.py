...
from branch.branch.report.stock_ledger_branch.stock_ledger_branch import get_item_group_condition
from erpnext.stock.utils import add_additional_uom_columns, is_reposting_item_valuation_in_progress

def company_branch(filters):
	if filters.get("company_branch"):
		company_branch += " and sle.company_branch = %s" % frappe.db.escape(filters.get("company_branch"))
...
def execute(filters=None):
	...
	for (company, item, warehouse, company_branch) in sorted(iwb_map):
		if item_map.get(item):
			qty_dict = iwb_map[(company, item, warehouse, company_branch)]
...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Company Branch"), "fieldname": "company_branch", "fieldtype": "Link", "options": "Company Branch", "width": 150}
	]
...
def get_conditions(filters):
	...
	if filters.get("company_branch"):
		conditions += " and sle.company_branch = %s" % frappe.db.escape(filters.get("company_branch"))	
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql("""
		select
			...
			sle.company,sle.company_branch, sle.voucher_type, sle.qty_after_transaction, sle.stock_value_difference,
...
def get_item_warehouse_map(filters, sle):
	...
	for d in sle:
		key = (d.company, d.item_code, d.warehouse,d.company_branch)
		...
		qty_dict = iwb_map[(d.company, d.item_code, d.warehouse,d.company_branch)]
...
def filter_items_with_no_transactions(iwb_map, float_precision):
	for (company, item, warehouse,company_branch) in sorted(iwb_map):
		...
		if no_transactions:
			iwb_map.pop((company, item, warehouse,company_branch))
