...
from branch.branch.report.sales_register_branch.sales_register_branch import get_mode_of_payments
from branch.branch.report.item_wise_sales_history_branch.item_wise_sales_history_branch import (
...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		row.update({
			...
			'company_branch': d.company_branch,
			'project': d.project,
			'company': d.company,
...
def get_conditions(filters):
	...
	for opts in (("company", " and company=%(company)s"),
		...
		("company_branch", " and `tabSales Invoice`.company_branch = %(company_branch)s"),
	...
	if filters.get("company_branch"):
		conditions +=  """and ifnull(`tabSales Invoice`.company_branch, '') = %(company_branch)s"""
...
def get_group_by_conditions(filters, doctype):
	...
	elif filters.get("group_by") in ('Customer', 'Customer Group', 'Territory', 'Supplier', 'Company Branch'):
...
def get_items(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select
			...
			`tabSales Invoice`.company_branch,
...
def get_group_by_and_display_fields(filters):
	...
	elif filters.get('group_by') == 'Company Branch':
		group_by_field = 'company_branch'
		subtotal_display_field = 'item_code'
	else:
