...
def get_data(filters):
	...
	for record in sales_order_records:
		...
		row = {
			...
			"company_branch": record.get('company_branch'),
...
def get_conditions(filters):
	...
	if filters.get("company_branch"):
		conditions += "AND so.company_branch = %s" %frappe.db.escape(filters.company_branch)
...
def get_sales_order_details(company_list, filters):
	...
	return frappe.db.sql("""
		SELECT
			...
			so.name,so.company_branch, so.transaction_date, so.customer,so.territory,
