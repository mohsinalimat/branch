...
erpnext.PointOfSale.ItemSelector = class {
		...
		this.wrapper.append(
			`<section class="items-selector">
				<div class="filter-section">
					<div class="label">${__('All Itemsssssss')}</div>
	...
	get_items({start = 0, page_length = 40, search_term=''}) {
		...
		return frappe.call({
			method: "branch.branch.branch.page.point_of_sale_branch.point_of_sale_branch.get_items",
			freeze: true,
	...
	get_item_html(item) {
		...
		if (item.is_stock_item) {
			indicator_color = (actual_qty > 10 ? "green" : actual_qty <= 0 ? "red" : "yellow");
	...
	make_search_bar() {
		...
		this.item_group_field = frappe.ui.form.make_control({
			df: {
				...
				get_query: function () {
					return {
						query: 'branch.branch.page.point_of_sale_branch.point_of_sale_branch.item_group_query',
						filters: {
